...
- **C1 待执行**：仍需在真实用户机器的 ComfyUI 中完成安装、模板导入、T2V/I2V 运行及 VIDEO+IMAGE+AUDIO 输出验收。
- **C2 未开始**：尚未创建 `v0.1.0` GitHub Release，也尚未发布到 ComfyUI Registry。
- 服务端仍保留在原工作区；S0 及后续服务端拆仓任务尚未执行。
