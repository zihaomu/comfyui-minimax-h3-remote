...
- **X1 发布候选已完成**：客户端已升至 `0.1.1`，新增 API v1/server 0.1.x 兼容预检、兼容矩阵和可执行联合 smoke；最终真实联合任务 `20260908T084429Z_4edb0cdb0026` 完成 22/22 唯一帧与 32 kHz 双声道，664,167 字节，下载 SHA 与服务端指标一致。客户端 ZIP 已生成并完成内容/凭据检查；服务端已补兼容矩阵、CHANGELOG、OpenAPI CI artifact 和 `server-v0.1.0` release manifest。待两仓库 CI 通过后创建 GitHub Releases；服务端镜像保持本地，不推送受保护 Registry。
