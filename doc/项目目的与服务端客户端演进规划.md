...
- **S3 进行中**：真实运行中取消已通过；GPU1 上的 960x544x362、20-step T2V 在 3.793 秒内完成取消闭环，任务 `20260908T060809Z_9c6aadb334ba` 终态为 `canceled`，PID 清空且无残留 `sd-cli`。断连重取也已通过：提交进程退出后，新进程仅凭 job ID 取回 `20260908T060856Z_080c7dc84fa3`，下载 SHA 与服务端一致且 22/22 帧唯一；异常重启恢复也已通过：强制终止容器后任务 `20260908T060956Z_0dd6d7f5af3d` 在持久化目录重启时被明确恢复为 `error`，且无孤儿 `sd-cli`；完整 362 帧生成和 live TTL 清理继续执行。
