...
- **X1 已完成**：客户端 `v0.1.1` 与私有服务端源码 `server-v0.1.0` GitHub Releases 已发布并回下载验证；兼容矩阵锁定 client 0.1.x / API v1 / server 0.1.x，最终联合任务 `20260908T084429Z_4edb0cdb0026` 完成 22/22 唯一帧与 32 kHz 双声道，下载 SHA 与服务端指标一致。客户端 ZIP、服务端 OpenAPI 和 release manifest 的 SHA 均已记录，两仓库发布提交 CI 成功。服务端镜像保持本地且未推送受保护 Registry；ComfyUI Registry 仍因缺少 `REGISTRY_ACCESS_TOKEN` 待办。
